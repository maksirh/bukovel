...
`scripts/start.sh` при **кожному старті** контейнера викликає `bootstrap_once` (один раз на БД), потім gunicorn. Bootstrap працює з повним prod-оточенням (SECRET_KEY, Cloudinary, DATABASE_URL).
...
На Render `bootstrap_once` викликається через `scripts/start.sh` при старті контейнера (не під час build).
Повторні deploy **не перезаписують** контент — мітка в `SiteBootstrap` + наявність HeroSlide.
